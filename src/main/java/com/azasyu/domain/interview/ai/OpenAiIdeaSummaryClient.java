...
public class OpenAiIdeaSummaryClient implements IdeaSummaryAiClient {
    ...
    @Override
    public IdeaSummaryDraft generate(Meeting meeting, List<IdeaCard> cards) {
        try {
    ...
    private String input(Meeting meeting, List<IdeaCard> cards) {
        String cardText = cards.stream().map(card -> """
            핵심 의견: %s
            이유: %s
            우려: %s
            대안: %s
            """.formatted(card.getCoreOpinion(), card.getRationale(), card.getConcern(), card.getAlternative()))
